...
if (isMobile.any()) {
	alert('Halo Sayang, pastikan  volume suaranya Sudah Full ya');
}
...
content.addEventListener('click', function () {
	...
	if (scene == 0) {
		...
		text.innerHTML = 'pohon ini cerita nya pernah Mas lihat di mimpi Mas';
		play();
	} else if (scene == 5) {
		text.innerHTML = 'emang gak mirip 100% sih, tapi emang buah-buah nya tu warna-warni dan Cantik banget';
		play();
	} else if (scene == 6) {
		text.innerHTML = 'abstrak tapi cantik bget, jadi Mas mau tunjukin ke Ici';
		play();
	} else if (scene == 7) {
		text.innerHTML = 'coba deh, luangin waktu bentar buat ngeliatin Pohon ini';
		play();
	} else if (scene == 8) {
		text.innerHTML = 'suka gak sayang?';
		play();
	} else if (scene == 9) {
		...
		const actionInterval = setInterval(function () {
			if (count == 3) {
				...
				text.innerHTML = 'gimana? cantik gak? Pasti Cantiknya Mirip Ici';
			}
			count++;
		}, 500);
		...
		text.innerHTML = 'tapi keliatan nya masih biasa aja ya sayang?';
		play();
	} else if (scene == 18) {
		...
		text.style.fontSize = '18px';
		text.style.top = '23%';
		text.innerHTML = 'teruntuk Ici, selamat ulang tahun ya';
		play();
	} else if (scene == 24 && count == -5) {
		text.style.fontSize = '18px';
		text.style.top = '23%';
		text.innerHTML = 'sorry kado Pertamanya ini dulu, Sisanya Nyusul ';
		play();
	} else if (scene == 25 && count == -5) {
		text.style.fontSize = '18px';
		text.style.top = '23%';
		text.innerHTML = 'kado kedua Mas kasih 300 bintang aja ya...';
		...
		text.style.fontSize = '18px';
		text.style.top = '23%';
		text.innerHTML = 'tuh Mas kasih bintang jatuh satu';
		...
		text.innerHTML = 'tuhhh, buruan make a wish sayang';
		shoots.classList = 'shootingStar one';
		play();
	} else if (scene == 32) {
		text.innerHTML = 'WKWK Ici wish apa? Atau gak sempet?';
		...
		text.innerHTML = 'oke deh, Mas kasih lagi, mau gak?';
		yes.innerHTML = 'mauuu';
		no.innerHTML = 'mau banget';
		btn.style.display = 'flex';
		play();
	} else if (scene == 35) {
		text.innerHTML = 'tuhhh Mas spam-in bintang jatuh buat Ici';
		btn.style.display = 'none';
		shoots.classList = 'shootingStar';
		shoots.innerHTML = '<span></span><span></span><span></span><span></span><span></span>';
		play();
	} else if (scene == 36) {
		text.innerHTML = 'biar Ici bisa make wishes sebanyak mungkinnn';
		play();
	} else if (scene == 37) {
		text.innerHTML = 'dan biar Mas gak dikatain pelit &#128540 HAHA';
		play();
	} else if (scene == 38) {
		text.innerHTML = '+ 1 lagi...';
		play();
	} else if (scene == 39) {
		text.innerHTML = 'Mas mau bilang';
		play();
	} else if (scene == 40) {
